...
/* DU */
static struct rcar_du_encoder_data gose_du_encoders[] = {
	{
...
static struct rcar_du_crtc_data gose_du_crtcs[] = {
	{
		.exclk = 148500000,
...
static struct rcar_du_platform_data gose_du_pdata = {
	.encoders = gose_du_encoders,
	.num_encoders = ARRAY_SIZE(gose_du_encoders),
	.crtcs = gose_du_crtcs,
	.num_crtcs = ARRAY_SIZE(gose_du_crtcs),
...
static void __init gose_add_du_device(void)
{
	struct platform_device_info info = {
		.name = "rcar-du-r8a7793",
		.id = -1,
		.res = du_resources,
		.num_res = ARRAY_SIZE(du_resources),
		.data = &gose_du_pdata,
		.size_data = sizeof(gose_du_pdata),
...
static const struct clk_name clk_names[] __initconst = {
	...
	{ "du0", "du.0", "rcar-du-r8a7793" },
	{ "du1", "du.1", "rcar-du-r8a7793" },
	{ "lvds0", "lvds.0", "rcar-du-r8a7793" },
...
static const struct sh_dmae_slave_config r8a7793_sys_dmac_slaves[] = {
...
static const struct sh_dmae_channel r8a7793_sys_dmac_channels[] = {
...
static struct sh_dmae_pdata r8a7793_sys_dmac_platform_data = {
	.slave		= r8a7793_sys_dmac_slaves,
	.slave_num	= ARRAY_SIZE(r8a7793_sys_dmac_slaves),
	.channel	= r8a7793_sys_dmac_channels,
	.channel_num	= ARRAY_SIZE(r8a7793_sys_dmac_channels),
...
static struct resource r8a7793_sys_dmac_resources[] = {
...
#define r8a7793_register_sys_dmac(id)				\
	platform_device_register_resndata(			\
		&platform_bus, "sh-dma-engine", 2 + id,		\
		&r8a7793_sys_dmac_resources[id * 3],	id * 1 + 3,	\
		&r8a7793_sys_dmac_platform_data,		\
		sizeof(r8a7793_sys_dmac_platform_data))

static void __init gose_add_dmac_prototype(void)
{
	r8a7793_register_sys_dmac(0);
	r8a7793_register_sys_dmac(1);
}
...
#define SCIF_AD(scif_type, index, baseaddr)		\
	OF_DEV_AUXDATA("renesas," scif_type "-r8a7793", baseaddr, \
...
static u32 gose_usbhs_pipe_type[] = {
...
static void __init gose_add_usb0_gadget(void)
{
...
static void __init gose_add_usb0_host(void)
{
...
static void __init gose_add_usb1_host(void)
{
...
static void __init gose_add_vin_device(unsigned idx,
...
#define GOSE_CAMERA(idx, name, addr, pdata, flag)			\
...
GOSE_CAMERA(0, "adv7612", 0x4c, NULL, RCAR_VIN_BT709);
GOSE_CAMERA(1, "adv7180", 0x20, NULL, RCAR_VIN_BT656);

static void __init gose_add_camera0_device(void)
{
	platform_device_register_data(&platform_bus, "soc-camera-pdrv", 0,
				      &cam0_link, sizeof(cam0_link));
	gose_add_vin_device(0, &vin0_pdata);
}

static void __init gose_add_camera1_device(void)
{
	platform_device_register_data(&platform_bus, "soc-camera-pdrv", 1,
				      &cam1_link, sizeof(cam1_link));
	gose_add_vin_device(1, &vin1_pdata);
}

/* VSP1 */
#if IS_ENABLED(CONFIG_VIDEO_RENESAS_VSP1) && \
!defined(CONFIG_DRM_RCAR_DU_CONNECT_VSP)
static const struct vsp1_platform_data gose_vsps_pdata __initconst = {
	.features = 0,
	.rpf_count = 5,
	.uds_count = 3,
	.wpf_count = 4,
...
static const struct vsp1_platform_data gose_vspd0_pdata __initconst = {
...
static const struct vsp1_platform_data gose_vspd1_pdata __initconst = {
	.features = VSP1_HAS_LIF,
	.rpf_count = 4,
	.uds_count = 1,
	.wpf_count = 4,
...
static const struct vsp1_platform_data * const gose_vsp1_pdata[] __initconst
									= {
	&gose_vsps_pdata,
	&gose_vspd0_pdata,
	&gose_vspd1_pdata,
...
static void __init gose_add_vsp1_devices(void)
{
	struct platform_device_info info = {
		.name = "vsp1",
		.size_data = sizeof(*gose_vsp1_pdata[0]),
		.num_res = 2,
		.dma_mask = DMA_BIT_MASK(32),
	};
	unsigned int i;
...
static void __init gose_add_usb_devices(void)
{
...
#define gose_add_msiof_device spi_register_board_info

static struct of_dev_auxdata gose_auxdata_lookup[] __initdata = {
	OF_DEV_AUXDATA("renesas,sdhi-r8a7793", 0xee100000, "sdhi0",
			&sdhi0_info),
	OF_DEV_AUXDATA("renesas,sdhi-r8a7793", 0xee140000, "sdhi1",
			&sdhi1_info),
	OF_DEV_AUXDATA("renesas,sdhi-r8a7793", 0xee160000, "sdhi2",
			&sdhi2_info),
	...
	OF_DEV_AUXDATA("renesas,msiof-r8a7793", 0xe6e20000,
		       "spi_r8a7793_msiof.0", &msiof0_info),
	OF_DEV_AUXDATA("renesas,msiof-r8a7793", 0xe6e10000,
		       "spi_r8a7793_msiof.1", &msiof1_info),
	{},
};

static void __init gose_add_standard_devices(void)
{
	...
	gose_add_du_device();
	gose_add_usb_devices();
	gose_add_camera0_device();
	gose_add_camera1_device();
#if IS_ENABLED(CONFIG_VIDEO_RENESAS_VSP1) && \
!defined(CONFIG_DRM_RCAR_DU_CONNECT_VSP)
	gose_add_vsp1_devices();
#endif
	gose_add_msiof_device(spi_bus, ARRAY_SIZE(spi_bus));
}

static const char * const gose_boards_compat_dt[] __initconst = {
	"renesas,gose",
	"renesas,gose-reference",
	NULL,
};

DT_MACHINE_START(GOSE_DT, "gose")
	.smp		= smp_ops(r8a7793_smp_ops),
	.init_early	= shmobile_init_delay,
	.init_time	= rcar_gen2_timer_init,
	.init_machine	= gose_add_standard_devices,
	.init_late	= shmobile_init_late,
	.reserve	= rcar_gen2_reserve,
	.restart	= gose_restart,
	.dt_compat	= gose_boards_compat_dt,
MACHINE_END
