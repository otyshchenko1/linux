...
static int jpu_try_fmt(struct file *file, void *priv, struct v4l2_format *f)
{
	...
	if (!v4l2_m2m_get_vq(ctx->m2m_ctx, f->type))
		return -EINVAL;
...
static int jpu_reqbufs(struct file *file, void *priv,
...
static int jpu_querybuf(struct file *file, void *priv,
...
static int jpu_qbuf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	struct jpu_ctx *ctx = fh_to_ctx(priv);
...
static int jpu_dqbuf(struct file *file, void *priv,
			  struct v4l2_buffer *buf)
{
	struct jpu_ctx *ctx = fh_to_ctx(priv);
...
static int jpu_expbuf(struct file *file, void *fh,
			   struct v4l2_exportbuffer *eb)
{
	struct jpu_ctx *ctx = fh_to_ctx(fh);
...
static int jpu_streamon(struct file *file, void *priv, enum v4l2_buf_type type)
{
	...
	if (adj.format.width != orig->format.width ||
		...
		dev_err(ctx->jpu->dev, "src and dst formats don't match.\n");
...
static int jpu_streamoff(struct file *file, void *priv,
			    enum v4l2_buf_type type)
{
	struct jpu_ctx *ctx = fh_to_ctx(priv);
...
static const struct v4l2_ioctl_ops jpu_ioctl_ops = {
	...
	.vidioc_reqbufs			= jpu_reqbufs,
	.vidioc_querybuf		= jpu_querybuf,
	.vidioc_qbuf			= jpu_qbuf,
	.vidioc_dqbuf			= jpu_dqbuf,
	.vidioc_expbuf			= jpu_expbuf,
	.vidioc_streamon		= jpu_streamon,
	.vidioc_streamoff		= jpu_streamoff,
...
static void jpu_buf_queue(struct vb2_buffer *vb)
{
	...
	if (!ctx->encoder && V4L2_TYPE_IS_OUTPUT(vb->vb2_queue->type)) {
		...
		/* check if JPEG data basic parsing was successfull */
...
static int jpu_buf_finish(struct vb2_buffer *vb)
{
	struct jpu_buffer *jpu_buf = vb2_to_jpu_buffer(vb);
	struct jpu_ctx *ctx = vb2_get_drv_priv(vb->vb2_queue);
...
static int jpu_stop_streaming(struct vb2_queue *vq)
{
	struct jpu_ctx *ctx = vb2_get_drv_priv(vq);
...
static int jpu_open(struct file *file)
{
	...
	ctx->m2m_ctx = v4l2_m2m_ctx_init(jpu->m2m_dev, ctx, jpu_queue_init);
...
static unsigned int jpu_poll(struct file *file,
...
static int jpu_mmap(struct file *file, struct vm_area_struct *vma)
{
	struct jpu *jpu = video_drvdata(file);
	struct jpu_ctx *ctx = fh_to_ctx(file->private_data);
	int ret;
...
static void jpu_cleanup(struct jpu_ctx *ctx, bool reset)
{
	...
	spin_unlock_irqrestore(&ctx->jpu->lock, flags);
	v4l2_m2m_job_finish(ctx->jpu->m2m_dev, ctx->m2m_ctx);
}
...
static irqreturn_t jpu_irq_handler(int irq, void *dev_id)
{
	...
	v4l2_m2m_job_finish(jpu->m2m_dev, curr_ctx->m2m_ctx);
...
static const struct of_device_id jpu_dt_ids[] = {
	{ .compatible = "renesas,jpu-gen2" }, /* H2/M2 */
	{ },
};
...
static int jpu_probe(struct platform_device *pdev)
{
	...
	strlcpy(jpu->vfd_encoder.name, DRV_NAME,
	...
	strlcpy(jpu->vfd_decoder.name, DRV_NAME,
